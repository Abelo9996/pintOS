...
tid_t thread_create(const char* name, int priority, thread_func* function, void* aux) {
  ...
  asm volatile("fsave (%0); fninit; fsave (%1); frstor (%2)"
               :
               : "g"(&temp_fpu), "g"(&(sf->fpu_stack)), "g"(&temp_fpu));

  struct execwait_connector* exec_connect = malloc(sizeof(struct execwait_connector));
  exec_connect->tid = tid;
  exec_connect->exit_code = -1;
  exec_connect->found = 0;
  exec_connect->ref_count = 2;
  exec_connect->file_copy = NULL;
  sema_init(&(exec_connect->exec_semaphore), 0);
  sema_init(&(exec_connect->wait_semaphore), 0);
  t->wait_connect = exec_connect;
  list_push_back(&running_thread()->children, &exec_connect->element);
...
void thread_set_priority(int new_priority) {
  ...
  if (new_priority > thread_current()->current_priority) {
  ...
  struct list_elem* thread_elem = list_max(&prio_ready_list, higher_priority_thread, NULL);
  struct thread* highest_prio_ready_thread = list_entry(thread_elem, struct thread, elem);
  if (highest_prio_ready_thread->current_priority > thread_current()->current_priority) {
    if (intr_context()) {
      intr_yield_on_return();
    } else {
      thread_yield();
...
int thread_get_priority(void) { return thread_current()->current_priority; }
...
bool higher_priority_thread(const struct list_elem* et1, const struct list_elem* et2, void* aux) {
  ...
  if (t1->current_priority < t2->current_priority) {
    return true;
  }
  return false;
}
...
static struct thread* thread_schedule_prio(void) {
  struct list_elem* largest_elem = NULL;
  struct thread* highest = NULL;
  if (!list_empty(&prio_ready_list)) {
    struct list_elem* pop = list_max(&prio_ready_list, higher_priority_thread, NULL);
    list_remove(pop);
    return list_entry(pop, struct thread, elem);
  } else {
    return idle_thread;
  }
}
/*Quicksort algorithm adopted from Beginners Book*/
...
static struct thread* thread_schedule_fair(void) {
  if (!list_empty(&fair_ready_list)) {
    int ready_list_size = list_size(&fair_ready_list);
    struct list_elem* thread_elem;
    struct thread* curr_thread;
    int priorities_arr[ready_list_size];
    int priorities_arr_index = 0;
    int num_of_each_prio[64];
    for (int i = 0; i < 64; i++) {
      num_of_each_prio[i] = 0;
    }
    for (thread_elem = list_begin(&fair_ready_list); thread_elem != list_end(&fair_ready_list);
         thread_elem = list_next(thread_elem)) {
      curr_thread = list_entry(thread_elem, struct thread, elem);
      priorities_arr[priorities_arr_index] = (curr_thread->current_priority) + 1;
      priorities_arr_index++;
      num_of_each_prio[(curr_thread->current_priority)]++;
    }
    quicksort(priorities_arr, 0, ready_list_size - 1);
    int priority_to_ticket_map[65];
    int total_tickets = 0;
    int jump_size = 1;
    int num_tickets = 1;
    int prev_prio = -1;
    ...
    for (int i = 0; i < ready_list_size; i++) {
      curr_prio = priorities_arr[i];
      if (curr_prio == prev_prio) {
        continue;
      }
      priority_to_ticket_map[curr_prio] = num_tickets;
      total_tickets += num_tickets;
      num_tickets += 1;
      prev_prio = curr_prio;
    }
    int random_num = ((int)(random_ulong() % (unsigned long)total_tickets)) + 1;
    int wanted_priority;
    int start = 1;
    int end = 0;
    prev_prio = -1;
    for (int i = 0; i < ready_list_size; i++) {
      ...
      end = start + priority_to_ticket_map[curr_prio] - 1;
      if (random_num >= start & random_num <= end) {
        wanted_priority = curr_prio;
        break;
      }
      start += priority_to_ticket_map[curr_prio];
      prev_prio = curr_prio;
    }
    int num_of_prio = num_of_each_prio[wanted_priority - 1];
    random_num = ((int)(random_ulong() % (unsigned long)num_of_prio));
    int find_count = 0;
    for (thread_elem = list_begin(&fair_ready_list); thread_elem != list_end(&fair_ready_list);
         thread_elem = list_next(thread_elem)) {
      curr_thread = list_entry(thread_elem, struct thread, elem);
      if (curr_thread->current_priority == wanted_priority - 1) {
        if (find_count == random_num) {
...
/* Helper function to determine thread with earlier wakeup time.
  If et1 is earlier than et2, return true, else return false. */
bool earlier_thread(const struct list_elem* et1, const struct list_elem* et2, void* aux) {
  struct thread* t1 = list_entry(et1, struct thread, sleep_elem);
  struct thread* t2 = list_entry(et2, struct thread, sleep_elem);
  if (et1 == NULL) {
    return false;
  }
  if (et2 == NULL) {
  ...
  if (t1->tick_to_wake < t2->tick_to_wake) {
    return true;
  }
  return false;
}